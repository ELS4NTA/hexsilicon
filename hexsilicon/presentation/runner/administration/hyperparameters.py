...
class Hyperparameters(ttk.Frame):
    ...
    def create_widgets(self):
        self.hyperparameters_frame = ttk.Labelframe(
            self, text="Hiperparámetros")

        self.btn_group = ttk.Frame(self)
        self.reset_btn = ttk.Button(
            self.btn_group, text="Reinicair", bootstyle="primary")
        self.update_btn = ttk.Button(
            self.btn_group, text="Actualizar", bootstyle="secondary")
        self.reset_btn.pack(side=LEFT, expand=YES, ipady=5, ipadx=15)
        self.update_btn.pack(side=LEFT, expand=YES, ipady=5, ipadx=15)

    def place_widgets(self):
        self.hyperparameters_frame.pack(
            side=TOP, expand=YES, fill=BOTH, padx=100, pady=100)
    ...
    def set_hyperparams(self, hyperparams):
        ...
        for hyperparameter, value in hyperparams.items():
            ...
            ttk.Scale(self.hyperparameters_frame,
                      from_=value[1], to=value[2], orient="horizontal", value=value[0]).pack()
